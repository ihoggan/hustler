...
class Sim:
    ...
    def __init__(self, layout="default"):
        ...
        # r23 bug 3: True (default) keeps the old SANDBOX behaviour -- a
        # potted cue ball is respotted immediately, so a bare Sim (drills,
        # the r5-r8 selftests, custom mode with no Game) is never left
        # without a cue ball. A Game sets this False on its own sim in
        # new_game(): rules-driven play must be able to remove the cue ball
        # and leave it OUT until the rules layer (ball_in_hand) and the
        # player decide where it goes -- Sim deliberately knows nothing
        # about Game, so this flag is how the caller opts out rather than
        # Sim reaching into rules state.
    ...
    def _capture_pockets(self):
        ...
        for bid, pos, radius in pots:
            ...
            if bid == self.CUE_ID:
                # r23 bug 3: only respot automatically in pure sandbox use
                # (auto_respot True). A Game-driven sim leaves the cue ball
                # OUT of the table entirely once potted -- Game.on_rest()
                # grants ball_in_hand and the player/AI places it, rather
                # than the sim silently deciding before the rules get a say.
                if self.auto_respot:
                    self._live_side = 0.0
                    self._live_follow = 0.0
                    self._respot_cue()
...
class Game:
    ...
    def legal_colours(self, sim, potted_this_shot=None):
        ...
        r23: potted_this_shot lets on_rest assess the shot that just finished
        against the table AT ADDRESS -- before this shot's own pots were
        removed. Without it, potting your LAST colour made remaining(own) read
        0 here, so the shot was judged as if you should have been on the
        black, and hitting your own colour was wrongly called a wrong-ball
        foul -- clearing your colours ended your turn instead of leaving you
        on the black. Called with no argument (live play, choosing the next
        shot), behaviour is unchanged."""
        own = self.own_colour()
        remaining_own = sim.remaining(own) if own else 0
        if potted_this_shot and own:
            remaining_own += sum(1 for c in potted_this_shot if c == own)
        if self.free_shot:
    ...
    def on_rest(self, sim):
        ...
        # assignment below mutates it. r23: pass potted_this_shot so a
        # cleared-out own colour still reads as "on" for THIS shot's
        # assessment, not as "should have been on the black" (bug 1).
        ...
        if foul is not None:
            ...
            self.free_shot = True
            self.visits_left = 2
            # r23 bug 3: WEPF blackball grants ball-in-hand-in-baulk to the
            # incoming player after ANY foul, not just a scratch -- this was
            # never granted at all beyond the initial break (line above sets
            # it False unconditionally on every completed shot, and nothing
            # ever set it back True). Previously masked because the sim
            # auto-respotted a potted cue ball on its own, so a scratch
            # LOOKED handled even though the rules layer was never actually
            # the one deciding.
            self.ball_in_hand = True
            return
...
class PoolAI:
    ...
    def place_cue(self, sim, legal_colours):
        ...
        Returns the chosen position, or None if no legal candidate scores."""
        rc = CFG["CUE_R_M"]
        ...
        if best_pos is None:
            return None
        # r23 bug 3: the cue ball may genuinely be OFF the table (potted,
        # auto_respot now False for a Game-driven sim) rather than merely
        # needing repositioning. _search above never touches an actual cue
        # body -- it's parameterised entirely by the candidate position -- so
        # placement itself only needs to handle the commit step here.
        cue = sim.cue()
        if cue is None:
...
def new_game(controllers=("ai", "ai"), names=("SHARK", "STEADY")):
    ...
    sim._respot_cue()
    sim.rack()
    # r23 bug 3: a Game-driven sim must NOT auto-respot a potted cue ball --
    # Game.on_rest() grants ball_in_hand instead, and the player/AI places it.
...
def shot_spin_and_reset(side, follow):
    """r23 bug 2: the spin a shot fires with, and what the HUD's spin state
    must fall back to immediately after -- dead centre, every time.

    Before this fix, do_shoot() consumed spin_side/spin_follow into the
    strike but never reset them, so whatever spin was dialled in on the
    SpinPad carried straight into the NEXT shot's aim (the Maker found bottom
    / draw spin stuck on and unable to de-select). Pulled out as a pure
    function so do_shoot()'s automatic reset and the existing manual
    Reset-spin button share exactly the same fallback value and can never
    drift apart, and so the fix is testable without the pygame wrapper.
    Returns (side_to_apply, follow_to_apply, new_side, new_follow)."""
...
def run_gui(smoke=False, smoke_frames=90, snap_path=None):
    ...
    def do_shoot():
        nonlocal pending, spin_side, spin_follow
        cue = sim.cue()
        if not shoot_enabled(cue is not None, sim.all_at_rest(), my_turn()):
            return
        dx, dy = rotate_vector(1.0, 0.0, aim_angle)
        # r23 bug 2: fire with the spin that was dialled in, then reset the
        # HUD's spin state to centre -- it must not carry into the next shot.
        side_to_apply, follow_to_apply, spin_side, spin_follow = \
            shot_spin_and_reset(spin_side, spin_follow)
        sim.strike((dx, dy), power, side=side_to_apply, follow=follow_to_apply)
        if not smoke:
                    ...
                    if not on_panel and ev.type in (pygame.MOUSEBUTTONDOWN,
                        ...
                        if pressed:
                            ...
                            others = [((b.position.x, b.position.y), s.radius)
                                      for bid, (b, s) in sim.balls.items()
                                      if bid != Sim.CUE_ID]
                            # r23 bug 3: the cue ball may genuinely be OFF the
                            # table (potted, auto_respot now False) rather
                            # than merely needing repositioning -- the old
                            # `cue is not None` guard silently ate every
                            # click/drag in that case. First press re-adds it
                            # at the clicked point; a drag afterwards moves
                            # the now-existing body, same as always.
...
def selftest():
    ...
    # 52. r23 bug 1 fix -- legal_colours must judge the shot against the table
    #     AT ADDRESS, not after this shot's own pots have already been
    #     removed. Before the fix, potting your LAST colour made
    #     sim.remaining(own) read 0 by the time on_rest checked it, so the
    #     game wrongly decided you should have been on the black and called
    #     your own colour a wrong-ball foul -- clearing your colours ended
    #     your turn instead of leaving you on the black.
    g52 = Game()
    g52.colours = {0: "red", 1: "yellow"}
    g52.current = 0
    # (a) pot the LAST red cleanly -> must stay on, no foul
    g52.on_rest(_FakeSimFull(first_contact="red", cushion=True,
                             potted=["red"], remaining=0))
    ok52_last = g52.current == 0 and "foul" not in g52.last_event
    # (b) pot a MID-GAME red (3 left after this pot) -> must stay on
    g52b = Game()
    g52b.colours = {0: "red", 1: "yellow"}
    g52b.current = 0
    g52b.on_rest(_FakeSimFull(first_contact="red", cushion=True,
                              potted=["red"], remaining=3))
    ok52_mid = g52b.current == 0 and "foul" not in g52b.last_event
    # (c) genuine wrong-ball-first, nothing potted -> still a foul, turn passes
    g52c = Game()
    g52c.colours = {0: "red", 1: "yellow"}
    g52c.current = 1        # on yellow
    g52c.on_rest(_FakeSimFull(first_contact="red", cushion=True,
                              potted=[], remaining=3))
    ok52_wrong = (g52c.current == 0 and g52c.fouls == 1
                 and "wrong ball first" in g52c.last_event)
    # (d) black potted legitimately (own colour cleared, black struck clean)
    #     -> striker wins
    g52d = Game()
    g52d.colours = {0: "red", 1: "yellow"}
    g52d.current = 0
    g52d.on_rest(_FakeSimFull(first_contact="black", cushion=True,
                              potted=["black"], remaining=0))
    ok52_black = g52d.over and g52d.winner == 0
    # (e) pot the last colour but scratch -> still a foul, turn passes
    g52e = Game()
    g52e.colours = {0: "red", 1: "yellow"}
    g52e.current = 0
    g52e.on_rest(_FakeSimFull(first_contact="red", cushion=True,
                              potted=["red", "cue"], remaining=0))
    ok52_scratch = (g52e.current == 1 and g52e.fouls == 1
                    and g52e.last_event == "foul — scratch")
    # (f) pot the last TWO reds in one shot -> must stay on
    g52f = Game()
    g52f.colours = {0: "red", 1: "yellow"}
    g52f.current = 0
    g52f.on_rest(_FakeSimFull(first_contact="red", cushion=True,
                              potted=["red", "red"], remaining=0))
    ok52_double = g52f.current == 0 and "foul" not in g52f.last_event
    check("r23 bug 1 — clearing your last colour cleanly stays on (no phantom "
          "wrong-ball foul), a mid-game pot still stays on, a genuine "
          "wrong-ball-first still fouls, a clean black still wins, a scratch "
          "on the last colour still fouls, and potting two at once still "
          "stays on",
          ok52_last and ok52_mid and ok52_wrong and ok52_black
          and ok52_scratch and ok52_double,
          f"last={ok52_last} mid={ok52_mid} wrong={ok52_wrong} "
          f"black={ok52_black} scratch={ok52_scratch} double={ok52_double}")

    # 53. r23 bug 2 fix -- spin must fire with whatever was dialled in, then
    #     fall back to dead centre so it can never carry into the NEXT shot.
    #     Before the fix, do_shoot() consumed spin_side/spin_follow into the
    #     strike but never reset them, so bottom/draw spin stayed selected
    #     shot after shot. shot_spin_and_reset() is the single source of
    #     truth do_shoot()'s automatic reset shares with the manual
    #     Reset-spin button.
    applied53 = shot_spin_and_reset(-0.7, 0.4)
    idle53 = shot_spin_and_reset(0.0, 0.0)
    ok53 = (applied53 == (-0.7, 0.4, 0.0, 0.0)
            and idle53 == (0.0, 0.0, 0.0, 0.0))
    check("r23 bug 2 — a shot fires with the spin that was set, then the HUD "
          "state resets to centre so it can't carry into the next shot",
          ok53, f"drawn spin -> {applied53}, no spin -> {idle53}")

    # 54. r23 bug 3 fix -- three coupled pieces of the same incident: the sim
    #     silently deciding cue placement before the rules/player got a say.
    #     (i) a Game-driven sim (auto_respot=False, set by new_game()) must
    #     leave a potted cue ball OFF the table, not silently respot it; a
    #     bare sandbox Sim (auto_respot defaults True) must still respot
    #     exactly as before -- this is an opt-out, not a behaviour change to
    #     the default. (ii) Game.on_rest() must grant ball_in_hand on ANY
    #     foul, not just a scratch -- this was NEVER granted before (only set
    #     True once, at Game.__init__, and never again), a gap the sim's old
    #     auto-respot masked because the table always looked "handled"
    #     regardless of what the rules layer decided. (iii) PoolAI.place_cue
    #     must be able to RE-ADD a genuinely absent cue ball, not just
    #     reposition an existing body -- _search never touches the cue body
    #     itself, so only the commit step needed this.
    pot54 = capture_points()[0][0]
    s54 = Sim(layout="empty")           # auto_respot defaults True (sandbox)
    s54._add_ball(s54.CUE_ID, (0.30, 0.455), "cue")
    s54.balls[s54.CUE_ID][0].position = pot54
    s54.step(1.0 / 60.0)
    ok54_sandbox_respots = s54.cue() is not None
    s54g = Sim(layout="empty")
    s54g.auto_respot = False            # r23: what new_game() sets
    s54g._add_ball(s54g.CUE_ID, (0.30, 0.455), "cue")
    s54g.balls[s54g.CUE_ID][0].position = pot54
    s54g.step(1.0 / 60.0)
    ok54_no_respot = (s54g.cue() is None and s54g.CUE_ID not in s54g.balls
                      and s54g.CUE_ID in s54g.potted_log)
    g54 = Game()
    g54.colours = {0: "red", 1: "yellow"}
    g54.current = 0
    g54.ball_in_hand = False
    g54.on_rest(_FakeSimFull(first_contact="red", cushion=True,
                             potted=["red", "cue"], remaining=3))
    ok54_bih_scratch = g54.ball_in_hand is True and g54.current == 1
    g54b = Game()
    g54b.colours = {0: "red", 1: "yellow"}
    g54b.current = 1
    g54b.ball_in_hand = False
    g54b.on_rest(_FakeSimFull(first_contact="red", cushion=True,
                              potted=[], remaining=3))    # wrong-ball, no scratch
    ok54_bih_wrongball = g54b.ball_in_hand is True
    s54c = Sim(layout="empty")
    s54c.auto_respot = False
    s54c._add_ball(s54c.alloc_id(), (0.9, 0.455), "red")
    ai54 = PoolAI("T", aim_jitter=0.0, threshold=0.05, greed=0.5, caution=0.5,
                 rng=random.Random(7))
    placed54 = ai54.place_cue(s54c, ["red"])
    ok54_ai_readd = placed54 is not None and s54c.cue() is not None
    ok54 = (ok54_sandbox_respots and ok54_no_respot and ok54_bih_scratch
            and ok54_bih_wrongball and ok54_ai_readd)
    check("r23 bug 3 — a Game-driven sim no longer auto-respots a potted cue "
          "ball (sandbox Sim still does), any foul grants ball_in_hand (not "
          "just a scratch, and not only once at game start), and the AI can "
          "re-place a genuinely missing cue ball",
          ok54,
          f"sandbox_respots={ok54_sandbox_respots} no_respot={ok54_no_respot} "
          f"bih_scratch={ok54_bih_scratch} bih_wrongball={ok54_bih_wrongball} "
          f"ai_readd={ok54_ai_readd}")
